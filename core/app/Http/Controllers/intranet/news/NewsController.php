...
class NewsController extends Controller
{
    ...
    public function get() {
        $query = News::select('news.id', 'news.title', 'news.active');
        ...
        return view('intranet/news/news-grid', ['items' => $items]);
    }

    public function getById($id) {
        $item = News::where('id', $id)->with('creator', 'updater')->first();

        if ($item == null)
            abort(404);

        $languages = $this->_languageService->GetArray();
        return view('intranet/news/news-form', ['item' => $item, 'languages' => $languages]);
    }
    ...
    public function set(Request $request)
    {
        ...
        return redirect()
            ->action([NewsController::class, 'getById'], ['id' => $item->id])
    ...
    public function delete($id) {
        ...
        {
            return redirect()
                ->action([NewsController::class, 'get'])
                ->with('message.error', 'No se ha podido eliminar el elemento indicado');
        }

        $item->delete();

        return redirect()->action([NewsController::class, 'get'])->with('message.success', 'Se ha eliminado la información correctamente');
    }
}
