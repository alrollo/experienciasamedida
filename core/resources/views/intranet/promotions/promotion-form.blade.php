...
@section('title', "Promoción - $item->title")
@section('meta_title', "Promoción - $item->title")
@section('meta_description', 'Promoción - $item->title')
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Promoción - {{ $item->title }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ url('intranet/dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ url('intranet/promotions') }}">Promociones</a></li>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form class="form-horizontal" method="post" action="{{ url("intranet/promotions/$item->id") }}">
                        {{ Form::token() }}
                        ...
                        <div class="card">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane active" id="informacion">
                                        <div class="row">
                                            ...
                                            <div class="col-3">
                                                <div class="icheck-primary">
                                                    <input type="checkbox" id="featured" name="featured" @if (old('featured', $item->featured)) checked @endif>
                                            ...
                                            <div class="col-6 col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group mb-3">
                                                        {{ Form::select('type_id', MasterTable::Get('tipos_promociones')->options->pluck('option', 'id'), old('type_id', $item->type_id), ['id' => 'type_id', 'class' => 'form-control', 'placeholder' => 'Elige un tipo...']) }}

                                                        @canany(['masters.update'])
                                                        <div class="input-group-append">
                                                            <button class="btn btn-outline-secondary btnAdminMasterOptions" data-id="{{ MasterTable::Get('tipos_promociones')->id }}"  data-name="{{ MasterTable::Get('tipos_promociones')->name }}" data-selectortorefresh="#type_id" type="button"><i class="fas fa-plus"></i></button>
                                                        </div>
                                                        @endcanany
                                                    </div>
                                                </div>
                                    ...
                                    @if($item->id!=0)
                                        <div class="tab-pane" id="imagenes">
                                                ...
                                                <div class="col-12 col-md-2 file-entity UploadImage"
                                                    ...
                                                    <div class="image-entity">
                                                        ...
                                                        <div class="image-container">
                                                            @if ($tagImage)
                                                                <a href="{{ asset('storage/promotion/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" target="_blank"><img src="{{ asset('storage/promotion/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" class="img-fluid"></a>
                                                            @else
                                                                <img src="{{ asset('assets/images/no_photo.png') }}" class="img-fluid">
                                                            @endif
                                                        </div>
                                                        <div class="buttons-container">
                                                            @can(['promotions.update', 'general.upload_files'])
                                                                <a href="#" class="btn btn-default btn-flat btnUploadImage" style="@if($tagImage) display: none; @endif"><i class="fas fa-plus"></i></a>
                                                            @endcan
                                                            @can(['promotions.update', 'general.edit_files'])
                                                            ...
                                                            @endcan
                                                            @can(['promotions.update', 'general.delete_files'])
                                            ...
                                            <hr class="divider">
                                            @can(['promotions.update', 'general.upload_files'])
                                                <div class="text-left mb-3"><a href="#" class="btn btn-outline-secondary btnUploadImages" data-id="{{ $item->id }}" data-model="Promotion"><i class="fas fa-plus"></i> Añadir Imágenes</a></div>
                                            @endcan
                                            ...
                                            @foreach($item->images->where('tag', null) as $image)
                                                <div class="col-12 col-md-2 file-entity" data-id="{{ $image->id }}">
                                                    <div class="image-entity">
                                                        <div class="image-container">
                                                            <a href="{{ asset('storage/promotion/'.$image->attachable_id.'/'.$image->file_name) }}" target="_blank"><img src="{{ asset('storage/promotion/'.$image->attachable_id.'/'.$image->file_name) }}" class="img-fluid"></a>
                                                        </div>
                                                        <div class="buttons-container">
                            ...
                            @canany(['promotions.create', 'promotions.update'])
...
@section('js')
    <script src="{{ asset('assets/js/intranet/promotions/PromotionForm.js') }}" type="module"></script>
@stop
