...
@section('page_class', 'page-works')

@section('title', "Trabajo - $item->title")
@section('meta_title', "Trabajo - $item->title")
@section('meta_description', 'Trabajo - $item->title')

@section('content-message')
@stop

@section('content-title')
    <a href="{{ url('intranet/news') }}" class="nav-link">Trabajos</a>
@stop
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Trabajo - {{ $item->title }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ url('intranet/dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ url('intranet/works') }}">Trabajos</a></li>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form class="form-horizontal" method="post" action="{{ url("intranet/works/$item->id") }}">
                        {{ Form::token() }}
                        ...
                        <div class="card">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane active" id="informacion">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6 col-md-4">
                                                <div class="form-group">
                                                    <label for="titleInput">Tags @if ($errors->has('tags_id')) <small class="text-danger">({{ $errors->first('tags_id') }})</small>@endif</label>

                                                    <div class="input-group mb-3">
                                                        {{ Form::select('tags[]', MasterTable::Get('tags_trabajos')->options->pluck('option', 'id'), old('tags[]', $item->tags->pluck('id')->toArray()), ['id' => 'tags', 'class' => 'form-control select2', 'multiple', 'data-placeholder' => 'Tags...']) }}

                                                        @canany(['masters.update'])
                                                        <div class="input-group-append">
                                                            <button class="btn btn-outline-secondary btnAdminMasterOptions" data-id="{{ MasterTable::Get('tags_trabajos')->id }}"  data-name="{{ MasterTable::Get('tags_trabajos')->name }}" data-selectortorefresh="#tags" type="button"><i class="fas fa-plus"></i></button>
                                                        </div>
                                                        @endcanany
                                                    </div>
                                                </div>
                                    ...
                                    @if($item->id!=0)
                                        <div class="tab-pane" id="imagenes">
                                                ...
                                                <div class="col-12 col-md-2 file-entity UploadImage"
                                                    ...
                                                    <div class="image-entity">
                                                        ...
                                                        <div class="image-container">
                                                            @if ($tagImage)
                                                                <a href="{{ asset('storage/works/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" target="_blank"><img src="{{ asset('storage/work/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" class="img-fluid"></a>
                                                            @else
                                                                <img src="{{ asset('assets/images/no_photo.png') }}" class="img-fluid">
                                                            @endif
                                                        </div>
                                                        <div class="buttons-container">
                                                            @can(['works.update', 'general.upload_files'])
                                                                <a href="#" class="btn btn-default btn-flat btnUploadImage" style="@if($tagImage) display: none; @endif"><i class="fas fa-plus"></i></a>
                                                            @endcan
                                                            @can(['works.update', 'general.edit_files'])
                                                            ...
                                                            @endcan
                                                            @can(['works.update', 'general.delete_files'])
                                            ...
                                            <hr class="divider">
                                            @can(['works.update', 'general.upload_files'])
                                                <div class="text-left mb-3"><a href="#" class="btn btn-outline-secondary btnUploadImages" data-id="{{ $item->id }}" data-model="News"><i class="fas fa-plus"></i> Añadir Imágenes</a></div>
                                            @endcan
                                            ...
                                            @foreach($item->images->where('tag', null) as $image)
                                                <div class="col-12 col-md-2 file-entity" data-id="{{ $image->id }}">
                                                    <div class="image-entity">
                                                        <div class="image-container">
                                                            <a href="{{ asset('storage/work/'.$image->attachable_id.'/'.$image->file_name) }}" target="_blank"><img src="{{ asset('storage/news/'.$image->attachable_id.'/'.$image->file_name) }}" class="img-fluid"></a>
                                                        </div>
                                                        <div class="buttons-container">
                            ...
                            @canany(['works.create', 'works.update'])
...
@section('js')
    <script src="{{ asset('assets/js/intranet/works/WorkForm.js') }}" type="module"></script>
