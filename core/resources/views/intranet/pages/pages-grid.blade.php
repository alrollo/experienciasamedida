...
@section('meta_title', 'Páginas')
@section('meta_description', 'Páginas')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            @canany(['pages.create'])
                            <a href="{{ url('intranet/pages/create') }}" class="btn btn-light btn-sm float-right"><i class="fas fa-plus"></i> Añadir</a>
                            @endcanany
                        </div>
                        ...
                        <div class="card-body">
                            <table id="tdItems" class="table table-bordered table-striped table-sm">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->name }}</td>
                                    <td>@if ($item->url) {!! implode(' | ', $item->url) !!} @endif</td>
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                            @canany(['pages.update'])
                                            <a href="{{ url("intranet/pages/{$item->id}") }}" class="btn btn-light"><i class="fas fa-pencil-alt"></i></a>
                                            @endcanany
                                            @canany(['pages.delete'])
                                            <a href="{{ url("intranet/pages/{$item->id}/delete") }}" class="btn btn-light btn-delete"><i class="fas fa-trash"></i></a>
                                            @endcanany
                                        </div>
                                    </td>
                                </tr>
