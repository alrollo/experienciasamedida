...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Membresía / Usuarios</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            @canany(['users.create'])
                            <a href="{{ url('intranet/membership/users/create') }}" class="btn btn-light btn-sm float-right"><i class="fas fa-plus"></i> Añadir</a>
                            @endcanany
                        </div>
                        ...
                        <div class="card-body">
                            <table id="tdItems" class="table table-bordered table-striped table-sm">
                                ...
                                <tr>
                                    ...
                                    <td class="@if($item->active == false) text-danger @endif">{{ $item->name }}</td>
                                    <td>{{ $item->email }}</td>
                                    <td>{{ join(', ', $item->roles->pluck('name_friendly')->toArray()) }}</td>
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                            <a href="{{ url("intranet/membership/users/{$item->id}") }}" class="btn btn-light"><i class="fas fa-pencil-alt"></i></a>
                                            <a href="{{ url("intranet/membership/users/{$item->id}/delete") }}" class="btn btn-light btnDelete"><i class="fas fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                @endforeach
...
@section('js')
    <script src="{{ asset('assets/js/intranet/membership/UsuarioForm.js') }}"></script>
@stop
