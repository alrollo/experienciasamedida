...
@section('page_class', 'page-products')

@section('title', "Familia - $item->title")
@section('meta_title', "Familia - $item->title")
@section('meta_description', 'Familia - $item->title')

@section('content-message')
@stop

@section('content-title')
    <a href="{{ url('intranet/products') }}" class="nav-link">Productos</a>
@stop
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Familia - {{ $item->title }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ url('intranet/dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ url('intranet/products') }}">Productos</a></li>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form class="form-horizontal" method="post" action="{{ url("intranet/products-families/$item->id") }}">
                        {{ Form::token() }}
                            ...
                            <div class="card-body">
                                ...
                                <div class="tab-content">
                                                    ...
                                                    @foreach ($languages as $lang)
                                                        <div class="tab-pane tab-pane-bordered fade @if($loop->first) show active @endif" id="contenido-{{$lang}}" role="tabpanel" aria-labelledby="home-tab">
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <textarea class="form-control" name="summary[{{$lang}}]" placeholder="Resumen">{{ old("summary.$lang", $item->translate('summary', $lang, false)) }}</textarea>
                                                            </div>
                                                ...
                                                <div class="col-12 col-md-2 file-entity UploadImage"
                                                    ...
                                                    <div class="image-entity">
                                                        ...
                                                        <div class="image-container">
                                                            @if ($tagImage)
                                                                <a href="{{ asset('storage/productfamily/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" target="_blank"><img src="{{ asset('storage/productfamily/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" class="img-fluid"></a>
                                                            @else
                                                                <img src="{{ asset('assets/images/no_photo.png') }}" class="img-fluid">
                                                            @endif
                                                        </div>
                                                        <div class="buttons-container">
                                                            @can(['products.update', 'general.upload_files'])
                                                                <a href="#" class="btn btn-default btn-flat btnUploadImage" style="@if($tagImage) display: none; @endif"><i class="fas fa-plus"></i></a>
                                                            @endcan
                                                            @can(['products.update', 'general.edit_files'])
                                                            ...
                                                            @endcan
                                                            @can(['products.update', 'general.delete_files'])
                                    ...
                                    <div class="tab-pane" id="seo">
                                        <div class="row">
                                            <div class="col-12">
                                                <ul class="nav nav-tabs" id="tabSeo" role="tablist">
                                                    @foreach ($languages as $lang)
                                                        <li class="nav-item">
                                                            <a class="nav-link @if($loop->first) active @endif" id="seo-{{$lang}}-tab" data-toggle="tab" href="#seo-{{$lang}}" role="tab" aria-controls="seo-{{$lang}}" aria-selected="true"><div class="fflag fflag-{{ strtoupper($lang) }} ff-sm" title="{{ $lang }}"></div></a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                                <div class="tab-content" id="myTabSeoContent">
                                                    @foreach ($languages as $lang)
                                                        <div class="tab-pane tab-pane-bordered fade @if($loop->first) show active @endif" id="seo-{{$lang}}" role="tabpanel" aria-labelledby="home-tab">
                                                            <div class="form-group">
                                                                <label for="roleInput">Title @if ($errors->has('title_seo')) <small class="text-danger">({{ $errors->first('title_seo') }})</small>@endif</label>
                                                                <input type="text" class="form-control" name="title_seo[{{$lang}}]" value="{{ old("title_seo.$lang", $item->translate('title_seo', $lang, false)) }}" />
                                                            </div>

                                                            <div class="form-group">
                                                                <label for="roleInput">Description @if ($errors->has('description_seo')) <small class="text-danger">({{ $errors->first('description_seo') }})</small>@endif</label>
                                                                <textarea class="form-control" name="description_seo[{{$lang}}]">{{ old("description_seo.$lang", $item->translate('description_seo', $lang, false)) }}</textarea>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
...
@section('js')
    <script src="{{ asset('assets/js/intranet/products/ProductFamilyForm.js') }}" type="module"></script>
@stop
