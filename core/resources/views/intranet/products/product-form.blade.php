...
@section('title', "Producto - $item->title")
@section('meta_title', "Producto - $item->title")
@section('meta_description', 'Producto - $item->title')
...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Producto - {{ $item->title }}</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form class="form-horizontal" method="post" action="{{ url("intranet/products/$item->id") }}">
                        {{ Form::token() }}
                        ...
                        <div class="card">
                                ...
                                <div class="tab-content">
                                        ...
                                        <div class="row">
                                            <div class="col-6 col-md-4">
                                                <div class="form-group">
                                                    <label for="titleInput">Familia @if ($errors->has('family_id')) <small class="text-danger">({{ $errors->first('family_id') }})</small>@endif</label>
                                                    {{ Form::select('family_id', $families->pluck('title', 'id'), old('family_id', $item->family_id), ['class' => 'form-control', 'placeholder' => 'Elige una familia...']) }}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    @if($item->id!=0)
                                        <div class="tab-pane" id="imagenes">
                                                ...
                                                <div class="col-12 col-md-2 file-entity UploadImage"
                                                    ...
                                                    <div class="image-entity">
                                                        ...
                                                        <div class="image-container">
                                                            @if ($tagImage)
                                                                <a href="{{ asset('storage/product/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" target="_blank"><img src="{{ asset('storage/product/'.$tagImage->attachable_id.'/'.$tagImage->file_name) }}" class="img-fluid"></a>
                                                            @else
                                            ...
                                            <hr class="divider">
                                            @can(['products.update', 'general.upload_files'])
                                                <div class="text-left mb-3"><a href="#" class="btn btn-outline-secondary btnUploadImages" data-id="{{ $item->id }}" data-model="Product"><i class="fas fa-plus"></i> Añadir Imágenes</a></div>
                                            @endcan
                                            ...
                                            @foreach($item->images->where('tag', null) as $image)
                                                <div class="col-12 col-md-2 file-entity" data-id="{{ $image->id }}">
                                                    <div class="image-entity">
                                                        <div class="image-container">
                                                            <a href="{{ asset('storage/product/'.$image->attachable_id.'/'.$image->file_name) }}" target="_blank"><img src="{{ asset('storage/product/'.$image->attachable_id.'/'.$image->file_name) }}" class="img-fluid"></a>
                                                        </div>
                                                        <div class="buttons-container">
                            ...
                            @canany(['products.create', 'products.udpate'])
...
@section('js')
    <script src="{{ asset('assets/js/intranet/products/ProductsForm.js') }}" type="module"></script>
@stop
